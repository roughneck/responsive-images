module ResponsiveImages
  module ViewHelpers
    ...
    def responsive_image_tag image, options={}
      ...
      image_tag image_src, data_sizes.merge(options)
    end
    ...
    def alternative_sizes image, sizes
      ...
      sizes[:sizes].each do |size, value|
        if value.present?
          data_sizes["data-#{size}-src"] = sizes[:hotlink] || (value == :default ? image.url : image.send(value).url)
        else
          false
        end
      end
